                              ...
                              std::size_t startingPos) -> std::string_view
{
    ...
    if (t1Pos == text.npos)
        throw std::runtime_error("[C++] getTextBetween: token1 not found!");
    ...
    if (t2Pos == text.npos)
        throw std::runtime_error("[C++] getTextBetween: token2 not found!");
...
constexpr auto getTextBetween(std::string_view text, std::size_t startingPos, std::string_view token) -> std::string_view
{
    ...
    if (tokenPos == text.npos)
        throw std::runtime_error("[C++] getTextBetween: token not found!");
                            ...
                            std::size_t fromPosition, bool skipToken) -> std::size_t
{
    ...
    if (pos == text.npos)
        throw std::runtime_error("[C++] stringFinder: token not found!");
